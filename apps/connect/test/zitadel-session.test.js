...
import { ZITADEL_ERROR_CODES, getZitadelConfig } from "../src/config/zitadel.js";
import {
  assertAuthRequestId,
...
test("createPasswordSession maps 401 to credential failure", withEnv(requiredEnv, async () => {
  ...
  await assert.rejects(
    () => createPasswordSession("V2_real-id", { loginName: "alice", password: "x" }, { fetch: fetchMock }),
    { code: ZITADEL_ERROR_CODES.ZITADEL_REQUEST_FAILED }
  );
}));
...
test("finalizeAuthRequest returns callbackUrl", withEnv(requiredEnv, async () => {
  ...
  const result = await finalizeAuthRequest(
    "V2_real-id",
    { sessionId: "sess-1", sessionToken: "tok" },
    { fetch: fetchMock }
  );
...
test("finalizeAuthRequest surfaces 409 as AUTH_REQUEST_NOT_READY", withEnv(requiredEnv, async () => {
  ...
  await assert.rejects(
    () => finalizeAuthRequest("V2_real-id", { sessionId: "sess-1" }, { fetch: fetchMock }),
...
test("finalizeAuthRequest surfaces 403 as UNAUTHORIZED (session.link missing)", withEnv(requiredEnv, async () => {
  ...
  await assert.rejects(
    () => finalizeAuthRequest("V2_real-id", { sessionId: "sess-1", sessionToken: "tok" }, { fetch: fetchMock }),
